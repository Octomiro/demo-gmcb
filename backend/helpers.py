...
# Proof images root — configurable via env var so Docker can override
# the path to match its volume mount without touching source code.
LIVE_IMAGES_ROOT = Path(
    os.environ.get(
        "LIVE_IMAGES_ROOT",
        str(Path(__file__).resolve().parent.parent / "data" / "liveImages"),
    )
)
